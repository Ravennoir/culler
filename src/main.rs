...
impl ImageViewerApp {
    fn new(cc: &eframe::CreationContext<'_>, path: Option<PathBuf>, initial_fullscreen: bool) -> Self {
        ...
        if let Some(path) = path {
            ...
            // entry — so even a 5000-file folder completes in < 5 ms on any local drive.
            let files = scan_image_directory(&path);
            if files.is_empty() {
                ...
                app.image_order = (0..app.image_files.len()).collect();
                app.current_index = cur;
                ...
                // Stage 1 — full preview on a dedicated OS thread.
    ...
    fn handle_command(&mut self, cmd: &str, ctx: &egui::Context) -> String {
        ...
        match parts[0] {
            "status" => {
                ...
                format!(
                    "cur={} file={} cur_loaded={} loaded={} cache={} pending={} mem={:.1}MB total={}",
            ...
            "help" | "" => {
                "commands: status | perf | cache | next | prev | compare_add | open <path>".into()
            }
...
fn scan_image_directory(file_path: &Path) -> Vec<PathBuf> {
    ...
    let mut files: Vec<PathBuf> = entries
        .filter_map(|entry| {
            ...
            if all_supported_formats.iter().any(|fmt| path_str.ends_with(fmt)) {
                Some(path)
            } else {
                None
            }
